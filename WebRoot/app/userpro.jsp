...
<body>
  ...
  <section id="main" class="entire_width">
    <div class="container_12">
       <div class="grid_12">
       <h1 class="page_title">我发布的商品</h1>

       <table class="cart_product">
        <tr>
		     <th class="images">商品图片</th>
		     <th class="bg name" style="width:66px">商品名称</th>
		     <th class="edit" style="width:250px">商品描述</th>
		     <th class="bg price">现价</th>
		     <th class="edit">原价</th>
		     <th class="bg name">发布日期</th>
		     <th class="close" style="width:66px">删除商品</th>
	      </tr>
	   	<c:choose>
			<c:when test="${not empty requestScope.prods}">
				<c:forEach var="prod" items="${requestScope.prods}" step="2">
			      <tr>
				     <td class="images"><a href="<%=path %>/ProductsServlet?tag=toProA&pid=${prod.id}">
				     <img src="<%=path %>/pictures/${prod.img}" alt="${prod.pname }"></a></td>
				     <td class="bg name">${prod.pname }</td>
				     <td class="edit">${prod.description }</td>
				     <td class="bg price">${prod.currentprice}</td>
				     <td class="edit"">${prod.oldprice}</td>
				     <td class="bg name">${prod.releasedate}</td>
				     <td class="close"><a title="close" class="close" href="<%= path%>/ProductsServlet?tag=deleteA&id=${prod.id}"></a></td>
			      </tr>
	      		</c:forEach>
			</c:when>
			<c:otherwise>
	      ...
	      <tr>
		     <td colspan="7" class="cart_but">
			    <h3><a href="<%=path%>/ProductsServlet?tag=toAddPro">发布新商品</a></h3>
		     </td>
	      </tr>
       </table>
       </div><!-- .grid_12 -->
