  ...
  <section id="main" class="entire_width">
    <div class="container_12">
       ...
       <table class="cart_product">
	      <tr>
	      	 <th class="edit">商品图片</th>
		     <th class="bg name">商品名称</th>
		     <th class="edit">商品价格</th>
		     <th class="bg name">下单用户名</th>
		     <th class="edit">买家电话</th>
		     <th class="bg name">邮寄地址</th>
		     <th class="edit">交易方式</th>
		     <th class="bg name">订单开始时间</th>
		     <th class="edit">订单更新时间</th>
			 <th class="bg name">操作</th>
	      </tr>
	      <c:choose>
			<c:when test="${not empty requestScope.orders}">
				<c:forEach var="order" items="${requestScope.orders}" step="2">
			      <tr align="center">
			         <td class="edit">
			         	<img src="<%=path%>/pictures/${order.product.img}" width="150px" height="100px"/>
			         </td>
				     <td class="bg name">${order.product.pname }</td>
				     <td class="edit">${order.product.currentprice}</td>
				     <td class="bg name">${order.user.username}</td>
				     <td class="edit">${order.user.tel}</td>
				     <td class="bg name">${order.user.address}</td>
				     <td class="edit">${order.method}</td>
				     <td class="bg name">${order.begindate}</td>
				     <td class="edit">${order.enddate}</td>
				     <td class="bg name"><a href="<%=path%>/OrderServlet?tag=updateA&id=${order.id}">完成交易</a></td>
			      </tr>
	      		</c:forEach>
			</c:when>
			<c:otherwise>
